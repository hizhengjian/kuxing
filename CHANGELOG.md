...
- 修复命令行参数 `--max-rounds` 无法覆盖配置文件的问题
  ...
  - 修改 `cli.py` 中 `--max-rounds` 默认值为 `None`（区分用户是否传参）
  - 在 `scheduler.py` 中添加参数覆盖逻辑

### 文档
- 更新 `docs/08-troubleshooting.md`，添加 `--max-rounds` 参数覆盖问题的解决方案
- 更新 `README.md` 版本号到 0.5.1
- 更新 `docs/SUMMARY.md` 版本信息
- 完善项目文档结构和内容
...
- **10 个结构化 section**：执行标题、当前状态、任务规格、关键文件、工作流程、错误与修正、代码库文档、学习总结、关键结果、工作日志
- **自动提取**：从执行结果中自动提取信息并更新
- **效果**：加载时间减少 50%，Token 消耗减少 40%

#### 2. 记忆索引文件（MEMORY.md）⭐⭐⭐⭐

- **快速定位**：按类型分类组织（会话记忆、项目背景、知识沉淀）
- **自动更新**：每次执行后同步更新索引

#### 3. 记忆大小控制与 LLM 智能压缩⭐⭐⭐⭐

- **新增文件**：`llm_compressor.py`
- **自动压缩**：使用 LLM 智能压缩，保留重要信息
- **大小限制**：session.md 20KB、context.md 30KB、knowledge_base.md 25KB
...
- 新增 `tests/test_session_memory.py`：13 个测试用例
- **总测试数**：62 个（全部通过 ✅）

详见 [CHANGELOG_v0.5.0.md](CHANGELOG_v0.5.0.md)
