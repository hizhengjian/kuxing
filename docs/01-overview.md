...
| **中断恢复** | 自动检测并修复不一致状态 | v0.5.1+ |
| **reset 命令** | 重置状态，支持软重置和硬重置 | v0.5.1+ |
...
- 修复命令行参数 `--max-rounds` 无法覆盖配置文件的问题
  - 添加 `LoopQueue.set_max_rounds()` 方法
  - 改进参数传递逻辑，确保命令行参数优先级高于配置文件

**新增功能**：
- 中断自动恢复：Ctrl+C 中断后再次运行时自动检测并修复不一致状态
  - 自动将 "running" 状态的任务重置为 "pending"
  - 自动校正 current_round 与实际轮次文件的匹配
- `reset --confirm` 命令：重置轮次记忆（保留配置）
- `reset --confirm --clear-all` 命令：清空所有记忆（包括配置）
...
如果在循环模式中使用了 `Ctrl+C` 中断，再次运行时会自动检测并修复不一致状态：
- 自动将 "running" 状态的任务重置为 "pending"
- 自动校正 current_round 与实际轮次文件的匹配
...
# 清空所有记忆（包括配置）- 慎用！只清空指定项目
python cli.py --config examples/update-kuxing-docs.yaml reset --confirm --clear-all
```

> **注意**：`reset` 命令只重置**一个项目**，由 `--config` 指定。如果不指定 `--config`，则重置 memory 目录下找到的第一个项目。
