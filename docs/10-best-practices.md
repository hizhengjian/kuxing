...
下图展示了从项目组织到性能优化的完整最佳实践决策流程：

![最佳实践决策树](./images/best-practices-decision.svg)

**决策流程说明**：

1. **阶段1: 项目组织** - 根据项目数量选择合适的目录结构，定义清晰的任务拆分
2. **阶段2: 配置优化** - 根据任务类型（循环/串行）选择合适的配置参数
3. **阶段3: 记忆系统** - 根据记忆层次（会话/项目/全局）选择合适的记忆文件
4. **阶段4: 调试优化** - 根据问题类型（配置/执行/记忆）选择对应的排查方法
5. **阶段5: 性能优化** - 根据性能瓶颈（速度/内存）选择优化策略
