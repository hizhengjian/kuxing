...
下图展示了从遇到问题到解决问题的完整排查流程，帮助您快速定位问题类型并找到解决方案：

![问题排查流程图](./images/troubleshooting-flow.svg)

**流程说明**：

1. **安装问题排查**（浅青色）：检查 Python 版本、Claude CLI 安装、依赖安装
2. **配置问题排查**（浅黄色）：验证 YAML 格式、任务依赖、执行模式、路径
3. **执行问题排查**（浅绿色）：检查参数位置、任务顺序、并行冲突、循环条件
4. **记忆问题排查**（粉色）：验证记忆加载、优先级、文件格式
5. **性能问题排查**（淡紫色）：优化 Prompt、启用压缩、清理日志
